...
describe('TrackCloneTests', function()
  it('should clone a track with equal events', function()
    local original = Track()
    table.insert(original.events, NoteBeginEvent(0, 0, 60, 100))
    table.insert(original.events, NoteEndEvent(480, 0, 60, 0))
    local copy = original:clone()
    expect(copy == original).to.be_truthy()
  end)

  it('should produce an independent track clone', function()
    local original = Track()
    table.insert(original.events, NoteBeginEvent(0, 0, 60, 100))
    local copy = original:clone()
    table.insert(copy.events, NoteEndEvent(480, 0, 60, 0))
    expect(#original.events).to.be_equal_to(1)
    expect(#copy.events).to.be_equal_to(2)
  end)

  it('should produce independent event clones within track', function()
    local original = Track()
    table.insert(original.events, NoteBeginEvent(0, 0, 60, 100))
    local copy = original:clone()
    copy.events[1].time_delta = 240
    expect(original.events[1].time_delta).to.be_equal_to(0)
  end)
end)
